...
- Post-baseline metered usage is evaluated in the order in which user values
  were last changed.
- Exhausting a hard-stop cost-center or organization budget freezes every
  active user in that scope.
- Exhausting the enterprise hard-stop budget blocks users that need overage;
  users still fully served from an included pool remain served.
...
|---|---|
| Node unit tests | Calculation details, fair sharing, ordering, state normalization, and boundary behavior without a browser |
| Playwright E2E tests | Configuration workflows, browser persistence, rendered status/reasons/accounting, and multi-step transitions |
...
| `ordered-overage` | Three Business users at a 1,900-credit baseline; $1 CC budget | Deterministic served-to-metered-to-blocked transitions |
...
| MTR-03 | Exercise CC hard stop one below, at, and one above its cap | Below/at remain metered; above blocks every active CC member only |
| MTR-04 | Exercise organization hard stop one below, at, and one above its cap | Below/at remain metered; above blocks every active org member only |
| MTR-05 | Exercise enterprise hard stop one below, at, and one above its cap | Below/at remain metered; above blocks overage users but not pool-only users |
...
| MTR-09 | Configure CC, org, and enterprise caps together | The smallest applicable headroom controls; the reason identifies that scope |
| MTR-10 | Configure a zero-dollar hard-stop budget | The first metered credit is blocked for the applicable scope |
...
| SEQ-02 | Change Thierry from 1,900 to 1,960 | Thierry becomes `metered`; others stay `served`; CC budget usage is $0.60 |
| SEQ-03 | Change Matthieu from 1,900 to 1,940 | Thierry and Matthieu are `metered`; Philippe stays `served`; budget is exactly $1.00 |
| SEQ-04 | Change Philippe from 1,900 to 1,910 | Demand exceeds the CC cap; all three become `blocked` with the CC-budget reason |
| SEQ-05 | Repeat the changes in reverse order | Edit order follows browser actions; pool sharing stays fair; final scope freeze is order-independent |
...
| SEQ-10 | Reload after ordered edits | The same baseline, order, values, statuses, and reasons are restored |

### Dashboard and lifecycle

| ID | Setup and actions | Expected result |
|---|---|---|
| UI-01 | Mix served, metered, and blocked rows | Summary counts and total metered usage equal the rows |
...
- Every README flowchart branch and “Key Interactions & Gotchas” row maps to a
  scenario above.
- Every outcome assertion includes the status and relevant source/reason and
  accounting; status alone is insufficient.
- Multi-step browser tests demonstrate `served -> metered -> blocked` from a
...
- Hard-stop on/off and exact-cap boundaries are covered for cost-center,
  organization, and enterprise budgets.
